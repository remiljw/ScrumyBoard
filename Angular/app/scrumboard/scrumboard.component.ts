...
export class ScrumboardComponent implements OnInit {
  ...
  constructor(private router: Router, public dataservice: DataService,private dragula:DragulaService, private http : HttpClient) {
    ...
    this.dataservice.role = sessionStorage.getItem('fullname');
    this.dataservice.project = sessionStorage.getItem('project_id');
    this.dataservice.channel = sessionStorage.getItem('channel');
  ...
  editGoal(event){
    ...
    if (goal_name == null || goal_name == ''){
      ...
      this.http.put('http://' + this.dataservice.domain_name + '/api/scrumgoals/', JSON.stringify({'mode': 1,'goal_id': items[0], 'new_name': goal_name,'project_id': this.dataservice.project}), this.dataservice.authOptions).subscribe(
    data => {
